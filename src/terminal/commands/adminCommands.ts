...
export async function handleSet({ addLine, supabase }: CommandContext, args: string) {
  ...
  try {
    ...
    addLine({ type: 'output', text: ` <span class="t-green">✓ System configuration updated: ${sanitizeHTML(key)} = ${sanitizeHTML(value)}</span>` });
  } catch(e) {
...
export async function handleBan({ addLine, supabase }: CommandContext, args: string) {
  ...
  try {
    ...
    addLine({ type: 'output', text: ` <span class="t-green">✓ IP ${sanitizeHTML(ip)} has been permanently banned from the server.</span>` });
  } catch (e) {
...
export async function handleUnban({ addLine, supabase }: CommandContext, args: string) {
  ...
  try {
    ...
    addLine({ type: 'output', text: ` <span class="t-green">✓ IP ${sanitizeHTML(ip)} has been removed from the banlist.</span>` });
  } catch (e) {}
}
