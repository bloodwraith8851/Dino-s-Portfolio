...
export async function handleGuestbook({ addLine, addLines, supabase }: CommandContext) {
  ...
  try {
    ...
    if (data && data.length > 0) {
      const gbLines = data.map((entry: any) => ({
  ...
  try {
    ...
    if (data && data.length > 0) {
      for (const poll of data) {
        ...
        votes?.forEach((v: any) => { if (voteCounts[v.option_index] !== undefined) voteCounts[v.option_index]++; });
        ...
        const lines = poll.options.map((opt: string, i: number) => {
          ...
          return { type: 'output' as const, text: `  <span class="t-cyan">[${i}]</span> ${sanitizeHTML(opt).padEnd(12)} <span class="t-dim">${bar}</span> ${pct}% (${count})` };
        });
        addLines(lines, 100);
      }
...
export async function handleVote({ addLine, visitorName, supabase }: CommandContext, args: string) {
  ...
  const { error } = await supabase.from('poll_votes').insert([{ poll_id: pollId, voter_ip: visitorName || 'visitor', option_index: optionIndex }]);
  if (error) {
...
export async function handleTopScores({ addLine, addLines, supabase }: CommandContext) {
  ...
  try {
    ...
    if (data && data.length > 0) {
      const scoreLines = data.map((entry: any, i: number) => ({
        type: 'output' as const,
