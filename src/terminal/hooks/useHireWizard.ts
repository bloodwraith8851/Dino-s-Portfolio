...
export function useHireWizard(addLine: AddLine, supabase: any, visitorName: string) {
  ...
  const initiateHire = () => {
    ...
    addLine({ type: 'output', text: ` <span class="t-dim">Let's get some basic details. Type 'cancel' at any time to abort.</span>` });
    if (visitorName) {
      addLine({ type: 'output', text: ` <span class="t-dim">I see you're already registered as ${visitorName}. Press Enter to keep this name, or type a new one.</span>` });
    }
  ...
  const processHireInput = async (cmd: string) => {
    ...
    if (hireStep === 'name') {
      ...
      setHireStep('msg');
      addLine({ type: 'output', text: ` <span class="t-green">What would you like to discuss?</span> <span class="t-dim">(e.g. project details, full-time role)</span>` });
    } 
      ...
      if (cmd.toLowerCase() === 'y' || cmd.toLowerCase() === 'yes') {
        ...
        try {
          ...
          addLine({ type: 'output', text: ` <span class="t-green font-bold">✓ Message delivered successfully.</span>\\n <span class="t-dim">Rakesh will get back to you shortly at ${sanitizeHTML(hireData.email)}.</span>` });
        } catch {
          addLine({ type: 'output', text: ` <span class="t-red">✗ Transmission failed.</span> <span class="t-dim">Please email me directly at rakeshsarkar9711@gmail.com</span>` });
        }
      } else {
        setHireStep(null);
