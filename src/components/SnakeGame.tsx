  ...
  return (
    <div className="absolute inset-0 z-50 bg-[#0C0C0C]/90 backdrop-blur flex flex-col items-center justify-center p-4">
      <div className="flex justify-between w-full max-w-[400px] mb-4 font-mono text-sm uppercase">
        <span className="text-[#D7E2EA]">Score: <span className="text-green-400">{score}</span></span>
        <button onClick={onExit} className="text-red-400 hover:text-white transition-colors">[ESC] Exit</button>
      </div>
      ...
      {gameOver && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center font-mono">
          ...
          <p className="text-[#D7E2EA] mb-6">
            {saved ? "Score saved to global leaderboard!" : "Saving score..."}
          </p>
          <div className="flex gap-4">
            <button 
              onClick={() => { setSnake(INITIAL_SNAKE); setDir({x:0,y:-1}); setGameOver(false); setScore(0); setSaved(false); setFood({x:5,y:5}); }}
            ...
            <button 
              onClick={onExit}
              className="px-6 py-2 border border-white text-white hover:bg-white/10"
            >
              EXIT
            </button>
          </div>
